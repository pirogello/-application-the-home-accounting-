...
import individualFinance.gui.Handler.*;
...
public class MainMenu extends JMenuBar implements Refresh, EnableEditDelete {
    ...
    private void init() {
        ...
        //addMenuItem(file,fileHandler, Text.get("MENU_FILE_NEW"), Style.ICON_MENU_FILE_NEW, HedlerCode.MENU_FILE_NEW, KeyEvent.VK_N);
        //addMenuItem(file,fileHandler,Text.get("MENU_FILE_OPEN"), Style.ICON_MENU_FILE_OPEN, HedlerCode.MENU_FILE_OPEN, KeyEvent.VK_O);
        addMenuItem(file, fileHandler, Text.get("MENU_FILE_SAVE"), Style.ICON_MENU_FILE_SAVE, HandlerCode.MENU_FILE_SAVE, KeyEvent.VK_S);
        addMenuItem(file, fileHandler, Text.get("MENU_FILE_UPDATE_CURRENCIES"), Style.ICON_MENU_FILE_UPDATE_CURRENCIES, HandlerCode.MENU_FILE_UPDATE_CURRENCY);
    ...
    private JMenuItem addMenuItem(JMenu menu, Handler listener, String title, ImageIcon icon, String action, int key){
        ...
        if(key != 0) {
            KeyStroke keyShort = KeyStroke.getKeyStroke(key, Toolkit.getDefaultToolkit().getMenuShortcutKeyMaskEx());
            item.setAccelerator(keyShort);
        }
        menu.add(item);
        return item;
    }
